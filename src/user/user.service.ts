...
export class UserService {
  ...
  async createUser(newUser: UserDTO) {
    ...
    if (checkUser != null) {
      this.utils.sendException(Constants.userAlreadyExists, Constants.httpStatus403, UserService.name);
    } else {
  ...
  async getUser(id: string) {
    ...
    if (user) {
      ...
      this.utils.sendException(Constants.userNotFound, Constants.httpStatus404, UserService.name);
    }
  }
  ...
  async updateUser(user: UserUpdateDTO) {
    ...
    if (checkUserEmail != null && checkUserEmail._id.toString() !== oldUser._id.toString()) {
      this.utils.sendException(Constants.userWithThisEmail, Constants.httpStatus403, UserService.name);
    } else {
      ...
      if (updatedInfo.modifiedCount === 1 && updatedInfo.matchedCount === 1) {
        ...
        this.utils.sendException(Constants.userNotUpdated, Constants.httpStatus202, UserService.name);
      } else {
        this.utils.sendException(Constants.userNotFound, Constants.httpStatus404, UserService.name);
      }
    }
